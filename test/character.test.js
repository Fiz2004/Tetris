...
describe('isCanDirections', () => {
	...
	it('Проверяем движение влево', () => {
		...
		assert.deepEqual(character.isCanDirections([{ x: -1, y: 0 }], grid), [{ x: -1, y: 0 }]);
	});
	it('Проверяем движение влево у края', () => {
		character.position.x = 0;
		assert.equal(character.isCanDirections([{ x: -1, y: 0 }], grid), false);
	});
	...
	it('Проверяем движение влево у края 2-е движение', () => {
		...
		assert.equal(character.isCanDirections([{ x: -1, y: 0 }, { x: -1, y: 0 }], grid), false);
	});
});
...
describe('isCanMove', () => {
	...
	it('Проверяем движение влево', () => {
		character.position.x = 3;
		assert.deepEqual(character.isCanMove([[{ x: -1, y: 0 }]], grid), [{ x: -1, y: 0 }]);
	});
	it('Проверяем движение влево у края', () => {
		character.position.x = 0;
		assert.deepEqual(character.isCanMove([[{ x: -1, y: 0 }]], grid), [{ x: 0, y: 0 }]);
	});
	it('Проверяем движение вправо у края', () => {
		character.position.x = grid.space[0].length - 1;
		assert.deepEqual(character.isCanMove([[{ x: 1, y: 0 }]], grid), [{ x: 0, y: 0 }]);
	...
	it('Проверяем движение влево у края 2-е движение', () => {
		character.position.x = 1;
		assert.deepEqual(character.isCanMove([[{ x: -1, y: 0 }, { x: -1, y: 0 }]], grid), [{ x: 0, y: 0 }]);
	});
	it('Проверяем движение влево у препятствия слева', () => {
		character.position.x = 1;
		grid.space[grid.space.length - 1][0].element = 1;
		assert.deepEqual(character.isCanMove([[{ x: -1, y: 0 }, { x: -1, y: 0 }], [{ x: 0, y: -1 }, { x: -1, y: 0 }]], grid), [{ x: 0, y: -1 }, { x: -1, y: 0 }]);
	});
